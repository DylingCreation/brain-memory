...
每次新版本开发前，必须先完成**全维度摸底**：

1. **构建检查** — `npm run build` 确认零报错
2. **测试检查** — `npx vitest run` 确认全部通过
3. **代码质量** — `grep -rn "TODO|FIXME" src/` 确认无遗留
4. **依赖健康度** — `npm outdated` + `npm audit` 检查依赖状态
5. **Git 状态** — `git status` 确认工作树干净

摸底结论写入版本规划文档后，才可开始新版本开发。
...
> **v0.2.0+ 新增**：`npm run doctor` — CLI 诊断工具，一键检查环境/依赖/配置/DB 状态。
...
- **不同门控条件的用例不得混在同一 `describe` 块中**（如 LLM 依赖 vs 纯算法）
- 不依赖外部条件（如 LLM API）的测试，必须始终运行，不能被 `skipIf` 门控包裹
- SQLite 测试必须 close 连接再清理文件，beforeEach + afterEach 双保险
...
新功能开发完成后 24h 内，将开发记录写入 `.devdocs/开发记录/` 目录，含开发内容、测试、验收、使用示例、后续扩展。Bug 修复记录写入 `.devdocs/问题修复/`。详细模板见 `METHODOLOGY.md` §五。

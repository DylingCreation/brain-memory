...
v0.2.0 完成了对所有 SQL 查询路径的逐项审计：

| 指标 | 数据 |
|------|------|
| `.prepare()` 调用 | **89 处**，全部使用 `?` 占位符 |
| `.exec()` 调用 | **8 处**，全部为 DDL/事务控制（纯静态 SQL） |
| 发现漏洞 | **0** |
| 总体判定 | ✅ **100% 参数化** |
...
| 模式 | 位置 | 安全性 |
|------|------|--------|
| Scope 隔离子句拼接 | `src/scope/isolation.ts` | ✅ 仅生成 `?` 占位符，值通过 params 传递 |
| IN 占位符生成 | `vectorSearchWithScore`, `graphWalk` | ✅ 由 `ids.map(() => "?").join(",")` 生成 |
| FTS5 MATCH | `searchNodes` | ✅ MATCH 参数通过 `.all(ftsQuery)` 参数绑定 |

详见 `.devdocs/技术决策/2026-04-29-SQL参数化审计报告.md`。

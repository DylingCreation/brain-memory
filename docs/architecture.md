...
│  └──────────┘ └──────────┘ └────────┘ └──────────┘ └─────────┘ │
│  ┌──────────┐ ┌────────────────────────────────────────────────┐ │
│  │Maintenanc│ │  提取器/召回器/混合召回/重排序/准入/工作记忆    │ │
│  │ Service  │ │  (算法实现，被领域服务调用)                     │ │
│  └──────────┘ └────────────────────────────────────────────────┘ │
└────────────────────────┬────────────────────────────────────────┘
                         │
┌────────────────────────┴────────────────────────────────────────┐
...
┌────────────────────────┴────────────────────────────────────────┐
│                     💾 存储层                                   │
│    SQLite (唯一 IStorageAdapter): 6 张表 + FTS5 + 触发器         │
│    LanceDB (ISearchIndex 伴随索引): embedding → nodeId 映射       │
│    可随时从 SQLite 全量重建，仅用于语义搜索加速                   │
└─────────────────────────────────────────────────────────────────┘
```
...
| **依赖** | VectorSearch、FTS5、GraphWalk、PPR、DecayEngine、IntentAnalyzer、QueryExpander |
| **关键算法** | 四路种子获取 + 统一 PPR + 路径感知融合 + 时间敏感性偏向 |

**四路种子获取：**
1. **精确路径**（Precise）：向量/FTS5 → 社区扩展 → 种子
2. **泛化路径**（Generalized）：社区向量匹配 → 成员召回 → 种子
3. **语义路径**（Semantic）：LanceDB ISearchIndex 语义搜索 → 种子
4. **外部路径**（External）：USER.md / MEMORY.md 人工沉淀记忆 → 种子

**v2.0 增强：**
- **路径感知融合（D9）**：四路种子标记来源，多路命中节点 ×1.2 加权
- **时间敏感性偏向（D10）**：检测 "最新""最近""今天" 等词汇，recency 加权
- **前置信息量判断（D8）**：`shouldRecall()` 过滤 "好的""继续""嗯" 等低信息量消息
- **缓存失效改进（R2）**：内容级 MD5 hash 替代脏节点计数

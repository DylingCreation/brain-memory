...
const BASE_URL = process.env.TEST_LLM_BASE_URL ?? "https://dashscope.aliyuncs.com/compatible-mode/v1";
const MODEL = process.env.TEST_LLM_MODEL ?? "qwen3.6-plus";
...
describe.skipIf(!LLM_ENABLED)("Session Reflection (LLM)", () => {
  it("extracts insights from session data", async () => {
    const nodes = [
      ...
      { name: "docker-port-fix", category: "cases", type: "EVENT", content: "Docker 端口冲突解决：修�?container_port �?8080 �?8081" },
    ];
...
describe.skipIf(!LLM_ENABLED)("Reasoning (LLM)", () => {
  it("derives conclusions from related nodes", async () => {
    const nodes = [
      makeNode({ name: "service-a-dockerfile", type: "TASK", category: "tasks", content: "A 服务使用 Python+Flask+Docker，端�?8080" }),
...
describe("Fusion Candidate Detection", () => {
  it("finds similar node pairs from vector DB", () => {
    ...
    const now = Date.now();
    db.exec(`
      INSERT INTO bm_nodes VALUES ('n1','TASK','tasks','docker-port-fix','Fix Docker port conflict','Change container port from 8080 to 8081','active',1,'[]',null,0,0.5,0,0,'static','user',null,null,null,null,null,null,null,null,${now},${now});
      INSERT INTO bm_nodes VALUES ('n2','TASK','tasks','docker-port-fix-solution','Docker port fix solution','Change container port from 8080 to 8081','active',1,'[]',null,0,0.5,0,0,'static','user',null,null,null,null,null,null,null,null,${now},${now});
    `);

    // Add 8 filler nodes
    for (let i = 3; i <= 10; i++) {
      db.exec(`
        INSERT INTO bm_nodes VALUES ('n${i}','TASK','tasks','filler-node-${i}','Filler node ${i}','Unrelated content for node ${i}','active',1,'[]',null,0,0.5,0,0,'static','user',null,null,null,null,null,null,null,null,${now},${now});
      `);
    }
